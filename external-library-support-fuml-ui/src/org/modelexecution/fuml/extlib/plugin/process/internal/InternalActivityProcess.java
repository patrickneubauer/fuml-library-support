...
import org.modelexecution.fumldebug.debugger.process.internal.ActivityExecCommand;
...
public class InternalActivityProcess extends Process implements ExecutionEventListener {
	...
	private ActivityExecCommand createStepCommand(int executionID, ActivityNode activityNode) {
		return new ActivityExecCommand(executionID, activityNode, Kind.NEXT_STEP);
	}
	...
	public void performCommands() {
		ISafeRunnable runnable = new ISafeRunnable() {
			...
			public void run() throws Exception {
				...
				while ((nextCommand = cmdQueue.poll()) != null && !shouldSuspend() && !shouldTerminate()) {
					executionID = nextCommand.getExecutionID();
					nextCommand.execute(integrationLayer.getExecutionContext());
				}
			}
	...
	private void handleActivityRuntimeException(Throwable exception, int executionID) {
	...
	private boolean concernsThisProcess(Event event) {
		return isFirstActivityEntryEvent(event) || hasOwnExecutionIDs(event) || hasParentHavingCorrectExecutionID(event);
	}

	private boolean hasOwnExecutionIDs(Event event) {
		return rootExecutionID == getExecutionID(event) || lastExecutionID == getExecutionID(event);
	}
	...
	public void stepUntil(int executionID, TracePointDescription pointDescription) {
	...
	public void addExecutionEventListener(ExecutionEventListener listener) {
		integrationLayer.getExecutionContext().addEventListener(listener);
	}
	...
	public void addBreakpoint(ActivityNode node) {
		if (node != null) {
			...
			integrationLayer.getExecutionContext().addBreakpoint(breakpoint);
		}
	}

	public void removeBreakpoint(ActivityNode node) {
		if (node != null) {
			Breakpoint breakpoint = integrationLayer.getExecutionContext().getBreakpoint(node);
			if (breakpoint != null) {
				integrationLayer.getExecutionContext().removeBreakpoint(breakpoint);
			}
		}
	}
	...
	public Trace getTrace() {
		return integrationLayer.getExecutionContext().getTrace(rootExecutionID);
	}
}
